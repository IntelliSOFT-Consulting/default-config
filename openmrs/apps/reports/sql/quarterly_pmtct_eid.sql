SELECT   CONCAT('Reporting Month : ',quarter.month) AS 'Serial \#',
...
FROM
	...
	-- TODO Confirm if there is difference between HEI and EID  number
	...
	INNER JOIN obs dbs
        ...
        AND dbs.voided = 0
        AND MONTH(dbs.value_datetime) = MONTH(LAST_DAY(DATE_ADD('#startDate#', INTERVAL 0 MONTH)))
		AND YEAR(dbs.value_datetime) = YEAR(LAST_DAY(DATE_ADD('#startDate#', INTERVAL 0 MONTH)))
...
SELECT  NULL AS 'Serial \#',
		NULL AS 'Date DBS sample collected',
        NULL AS 'EID Number',
        NULL AS 'Sex',
        NULL AS 'Age at enrolment ' ,
        NULL AS '0-≤2 months of age',
...
FROM
	...
	INNER JOIN obs dbs
        ...
        AND dbs.concept_id = (SELECT cv.concept_id FROM concept_view cv where cv.concept_full_name= 'HEI Testing (First PCR Test Date)')
        AND dbs.voided = 0
        AND MONTH(dbs.value_datetime) = MONTH(LAST_DAY(DATE_ADD('#startDate#', INTERVAL 1  MONTH)))
		AND YEAR(dbs.value_datetime) = YEAR(LAST_DAY(DATE_ADD('#startDate#', INTERVAL 1  MONTH)))
	LEFT JOIN obs enrolled
		ON enrolled.person_id = p.person_id
        AND enrolled.concept_id = (SELECT cv.concept_id FROM concept_view cv where cv.concept_full_name= 'HEI Treatment - Enrolled AT ART Date')
        AND enrolled.voided = 0
...
FROM
	...
	INNER JOIN obs dbs
        ...
        AND dbs.concept_id = (SELECT cv.concept_id FROM concept_view cv where cv.concept_full_name= 'HEI Testing (First PCR Test Date)')
        AND dbs.voided = 0
        AND MONTH(dbs.value_datetime) = MONTH(LAST_DAY(DATE_ADD('#startDate#', INTERVAL 2 MONTH)))
		AND YEAR(dbs.value_datetime) = YEAR(LAST_DAY(DATE_ADD('#startDate#', INTERVAL 2 MONTH)))
	LEFT JOIN obs enrolled
		ON enrolled.person_id = p.person_id
        AND enrolled.concept_id = (SELECT cv.concept_id FROM concept_view cv where cv.concept_full_name= 'HEI Treatment - Enrolled AT ART Date')
        AND enrolled.voided = 0
